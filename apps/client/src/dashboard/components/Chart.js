...
const Chart = ({ selectedTimeRange, data }) => {
	...
	const groups = useMemo(() => {
		const days = [...Array(selectedTimeRange)].map((_, index) => getDayBefore(selectedTimeRange - index - 1));

		return days.map((day) => {
			const activities = data.filter((activity) => new Date(activity.date).toDateString() === day.toDateString());

			return Object.entries(ACTIVITIES_MAP).reduce(
				(result, [key, { title }]) => ({ ...result, [title]: filterByType(activities, key).length }),
				{ date: day.getTime() },
			);
		});
	...
	return (
		<Wrapper title="Activities Over Time">
			<AutoSizer disableHeight>
				{({ width }) => (
					<BaseChart width={width} height={245} data={groups} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
						<defs>
						...
						<XAxis dataKey="date" tickMargin={5} tickFormatter={(value) => dayjs(value).format(SHORT_DATE_FORMAT)} />
						<YAxis hide />
						<Tooltip isAnimationActive={false} labelFormatter={(value) => dayjs(value).format(LONG_DATE_FORMAT)} />
