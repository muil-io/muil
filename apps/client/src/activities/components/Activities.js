...
const Activities = () => {
	...
	const filters = useMemo(
		() => [
			{ label: 'All', value: 'all' },
			...Object.entries(ACTIVITIES_MAP).map(([key, { shortTitle }]) => ({ value: key, label: shortTitle })),
		],
		[],
	);
	...
	const filteredData = useMemo(
		() =>
			data.filter(
				({ type, status }) => selectedFilter === 'all' || selectedFilter === type || selectedFilter === status,
			),
		[data, selectedFilter],
	);
	...
	return (
		...
		>
			<Table
				items={filteredData}
				rowHeight={({ rowData: { type, status } }) => (type !== 'email' ? 48 : status === 'error' ? 80 : 60)}
				defaultSortBy="date"
				...
				noDataSubTitle={
					<>
						Visit our{' '}
						<a href="https://docs.muil.io/docs/api/authorization/" target="_blank" rel="noopener noreferrer">
							documentation
						</a>{' '}
			...
			>
				...
				<Column
					...
					showOnSize="mobile"
				/>
				<Column label="Template" dataKey="displayName" flexGrow={1.5} width={1} showOnSize="mobile" />
