...
import { flexColumn, Input as BaseInput, Header3, BodySmall, Button, Spinner } from 'shared/components';
...
const EmailForm = ({ dynamicProps, baseTemplateUrl }) => {
	const handleSubmitForm = useCallback(
		async ({ subject, to, cc, bcc }) => {
			try {
				await api.postWithToken(baseTemplateUrl, {
					subject,
					to,
					cc,
					bcc,
			...
			render={({ handleSubmit, submitting, submitError, submitSucceeded }) => (
				<FormContainer onSubmit={handleSubmit}>
					...
					<Field
						name="subject"
						validate={required}
						render={({ input, meta }) => <Input {...input} error={meta.error && meta.touched} placeholder="Subject" />}
					/>

					<Field
						name="to"
						validate={atLeastOneToFieldRequired}
						render={({ input, meta }) => <Input {...input} error={meta.error && meta.touched} placeholder="To" />}
					/>

					<Field
						name="cc"
						validate={atLeastOneToFieldRequired}
						render={({ input, meta }) => <Input {...input} error={meta.error && meta.touched} placeholder="CC" />}
					/>

					<Field
						name="bcc"
						validate={atLeastOneToFieldRequired}
						render={({ input, meta }) => <Input {...input} error={meta.error && meta.touched} placeholder="BCC" />}
					/>
