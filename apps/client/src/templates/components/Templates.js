...
const Templates = () => {
	...
	const { loading, data, deleteBranch } = templatesStore();
	const [selectedBranch, setSelectedBranch] = useState();
	const [selectedTemplate, setSelectedTemplate] = useState();

	const groupedByBranch = useMemo(() => groupBy(data || [], 'branch'), [data]);
	const branches = useMemo(() => Object.keys(groupedByBranch).map((branch) => ({ label: branch, value: branch })), [
		groupedByBranch,
	]);
	const templates = useMemo(() => groupedByBranch?.[selectedBranch] || [], [groupedByBranch, selectedBranch]);
		...
		>
			...
			<Table
				...
				noDataSubTitle={
					<>
						Visit our{' '}
						<a href="https://docs.muil.io/docs/configurations/cli/#publish" target="_blank" rel="noopener noreferrer">
							documentation
						</a>{' '}
